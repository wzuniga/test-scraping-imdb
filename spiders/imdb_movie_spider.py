...
from utils.imdb_utils import extract_next_data_json, extract_movie_item
import os
...
class ImdbMovieSpider(scrapy.Spider):
    """
    Scrapy spider to extract movie data from IMDb top movies.
    Handles retries, proxy logging, and yields movie items from detail pages.
    """
    name = 'imdb_movie'
    ...
    def parse_movie_detail(self, response):
        """Parse the movie detail page and yield movie items."""
        # Log proxy info if present
        proxy = response.meta.get('proxy') or response.request.meta.get('proxy')
        if proxy:
            self.logger.info(f'Using proxy: {proxy}')
